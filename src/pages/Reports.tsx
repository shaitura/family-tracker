...
export default function Reports() {
  ...
  const filtered = transactions.filter((t) => {
    if (!t.date.startsWith(`${year}-${month}`)) return false;
  ...
  const exportExcel = async () => {
    ...
    writeFile(wb, `family-report-${year}-${month}.xlsx`);
  };

  const months = Array.from({ length: 12 }, (_, i) => ({ val: String(i + 1).padStart(2, '0'), label: new Date(2000, i).toLocaleString('he', { month: 'long' }) }));
  ...
  return (
    <div className="space-y-4 animate-fade-in" dir="rtl">
      ...
      <Card>
        <CardContent className="pt-4 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <p className="text-xs text-white/50 mb-1">חודש</p>
              <select value={month} onChange={(e) => setMonth(e.target.value)} className="w-full h-9 rounded-xl border border-white/15 bg-white/5 px-3 text-sm text-white focus:outline-none">
                {months.map(({ val, label }) => <option key={val} value={val} className="bg-slate-800">{label}</option>)}
              </select>
            </div>
          </div>
      ...
      <div className="text-center">
        <p className="text-xs text-white/50">סה"כ</p>
