...
const PAY_LABELS: Record<PayType, string> = { none: 'ללא עלות', monthly: 'חודשי', onetime: 'חד-פעמי' };
...
function feeLabel(s: Subscription): string {
  const pt = payType(s);
  if (pt === 'none')    return 'ללא עלות';
  if (pt === 'monthly') return `${fmt(s.monthly_fee)}/חודש`;
  return `${fmt(s.monthly_fee)} חד-פעמי`;
}

/** Returns expiry badge using expiry_date (with fallback to legacy renewal_date) */
function expiryBadge(s: Subscription) {
  const date = s.expiry_date || s.renewal_date;
  if (!date) return null;
  const days = Math.ceil((new Date(date).getTime() - Date.now()) / 86400000);
  const d = new Date(date);
  const formatted = d.toLocaleDateString('he-IL', { day: 'numeric', month: 'short', year: '2-digit' });
  if (days < 0)   return { text: `פג תוקף (${formatted})`,     cls: 'text-red-400 bg-red-400/10 border-red-400/20' };
  if (days <= 30) return { text: `פג תוקף בעוד ${days} ימים`,  cls: 'text-orange-400 bg-orange-400/10 border-orange-400/20' };
  if (days <= 90) return { text: `תוקף עד ${formatted}`,        cls: 'text-amber-400 bg-amber-400/10 border-amber-400/20' };
  return           { text: `תוקף עד ${formatted}`,              cls: 'text-white/30 bg-white/5 border-white/10' };
}
...
const EMPTY: Omit<Subscription, 'id'> = {
  ...
  payment_date: '', expiry_date: '', start_date: '',
  card_number: '', image_url: '', notes: '', is_active: true,
};

const INPUT = 'w-full bg-white/5 border border-white/10 rounded-xl p-2.5 text-sm text-white placeholder:text-white/25 outline-none focus:border-cyan-500/40';
...
export default function Subscriptions() {
  ...
  const { mutate: del } = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['subscriptions'] }); toast({ title: 'נמחק' }); },
  });
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 pb-24 space-y-4">
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי שם או ספק..."
        ...
        {search && <button onClick={() => setSearch('')} className="absolute top-1/2 -translate-y-1/2 left-3 text-white/30 hover:text-white/60"><X size={12} /></button>}
      </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1">
        {(['all', ...SUB_CATEGORIES] as const).map(c => (
          <button key={c} onClick={() => setCatFilter(c)}
            className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${catFilter === c ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' : 'text-white/40 border-transparent hover:text-white/60'}`}>
        ...
        {(['all', 'Shi', 'Ortal', 'Joint'] as const).map(o => (
          <button key={o} onClick={() => setOwnerFilter(o)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
              ownerFilter === o
                ? o === 'all' ? 'bg-white/10 text-white border-white/20' : o === 'Shi' ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30' : o === 'Ortal' ? 'bg-purple-500/20 text-purple-300 border-purple-500/30' : 'bg-pink-500/20 text-pink-300 border-pink-500/30'
        ...
        {grouped.map(({ cat, items, monthlySum, onetimeCount }) => (
          <div key={cat} className="space-y-2">
            <div className="flex items-center justify-between px-1 border-b border-white/5 pb-2">
              ...
              <div className="flex items-center gap-2">
                {monthlySum > 0 && <span className="text-xs font-medium text-emerald-400">{fmt(monthlySum)}<span className="text-white/30">/חודש</span></span>}
                {onetimeCount > 0 && <span className="text-xs text-amber-400/70">{onetimeCount} חד-פעמי</span>}
                {monthlySum === 0 && onetimeCount === 0 && <span className="text-xs text-white/25">ללא עלות</span>}
              </div>
            </div>

            {items.map(sub => {
              const eb = expiryBadge(sub);
              ...
              return (
                <div key={sub.id} className="flex gap-3 p-3 rounded-xl bg-white/5 border border-white/8 active:bg-white/10 transition-colors">
                  <div className="shrink-0 w-12 h-12 rounded-xl overflow-hidden bg-white/8 flex items-center justify-center border border-white/8">
                    {sub.image_url ? <img src={sub.image_url} alt={sub.name} className="w-full h-full object-cover" /> : <span className="text-xl">{CAT_META[sub.category as SubCategory]?.emoji ?? '📦'}</span>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex gap-0.5 shrink-0">
                        <button onClick={() => openEdit(sub)} className="text-white/25 hover:text-white/60 p-1.5 rounded-lg hover:bg-white/5 transition-colors"><Pencil size={13} /></button>
                        <button onClick={() => { if (confirm(`למחוק את "${sub.name}"?`)) del(sub.id); }} className="text-white/25 hover:text-red-400 p-1.5 rounded-lg hover:bg-red-500/10 transition-colors"><Trash2 size={13} /></button>
                      </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-md border ${om.cls}`}>{om.label}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-md border ${pt === 'monthly' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : pt === 'onetime' ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-white/5 border-white/10 text-white/40'}`}>
                        {feeLabel(sub)}
                      </span>
                      {eb && <span className={`text-[10px] px-1.5 py-0.5 rounded-md border ${eb.cls}`}>{eb.text}</span>}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm"
          ...
          <div dir="rtl" className="w-full max-w-md bg-slate-900 border border-white/10 rounded-t-2xl sm:rounded-2xl p-5 space-y-4 max-h-[92vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder='למשל: מטמון, H&M Club, Netflix' className={INPUT} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input value={form.provider} onChange={e => setForm(f => ({ ...f, provider: e.target.value }))} placeholder='למשל: רשות הטבע' className={INPUT} />
                </div>
                <div>
                  <label className="text-xs text-white/50 block mb-1">מספר כרטיס/מנוי</label>
                  <input value={form.card_number || ''} onChange={e => setForm(f => ({ ...f, card_number: e.target.value }))} placeholder='אופציונלי' className={INPUT} />
                </div>
              </div>
                ...
                <div className="flex rounded-xl border border-white/10 overflow-hidden">
                  {(['none', 'monthly', 'onetime'] as const).map(pt => (
                    <button key={pt} onClick={() => setPaymentType(pt)}
                      className={`flex-1 py-2.5 text-xs font-medium transition-colors border-l border-white/10 last:border-l-0 ${currentPT === pt ? 'bg-cyan-500/20 text-cyan-300' : 'text-white/40 hover:text-white/60'}`}>
                      {PAY_LABELS[pt]}
                    </button>
                  ))}
                </div>
              ...
              {/* ── Fields per payment type ── */}

              {/* MONTHLY: עלות חודשית + מועד תשלום + תוקף */}
              {currentPT === 'monthly' && (
                <>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs text-white/50 block mb-1">עלות חודשית (₪)</label>
                      <input type="number" min="0" value={form.monthly_fee}
                        onChange={e => setForm(f => ({ ...f, monthly_fee: Number(e.target.value) }))} className={INPUT} />
                    </div>
                    <div>
                      <label className="text-xs text-white/50 block mb-1">מועד תשלום הבא</label>
                      <input type="date" value={form.payment_date || ''}
                        onChange={e => setForm(f => ({ ...f, payment_date: e.target.value }))} className={INPUT} />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs text-white/50 block mb-1">תוקף מנוי / כרטיס</label>
                    <input type="date" value={form.expiry_date || ''}
                      onChange={e => setForm(f => ({ ...f, expiry_date: e.target.value }))} className={INPUT} />
                  </div>
                </>
              )}

              {/* ONETIME: סכום + מועד תשלום + תוקף */}
              {currentPT === 'onetime' && (
                <>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs text-white/50 block mb-1">סכום התשלום (₪)</label>
                      <input type="number" min="0" value={form.monthly_fee}
                        onChange={e => setForm(f => ({ ...f, monthly_fee: Number(e.target.value) }))} className={INPUT} />
                    </div>
                    <div>
                      <label className="text-xs text-white/50 block mb-1">מועד התשלום</label>
                      <input type="date" value={form.payment_date || ''}
                        onChange={e => setForm(f => ({ ...f, payment_date: e.target.value }))} className={INPUT} />
                    </div>
                  </div>
              ...
              {/* NONE: תחילת מנוי + תוקף */}
              {currentPT === 'none' && (
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-white/50 block mb-1">תחילת מנוי / כרטיס</label>
                ...
                <div className="flex items-center gap-2.5">
                  {form.image_url && (
                    <div className="relative w-20 h-12 rounded-lg overflow-hidden border border-white/10 shrink-0">
                      ...
                      <button onClick={() => setForm(f => ({ ...f, image_url: '' }))} className="absolute top-0.5 left-0.5 bg-black/70 rounded-md p-0.5">
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setDialog(null)} className="flex-1 py-2.5 rounded-xl border border-white/10 text-sm text-white/60 hover:text-white/80 transition-colors">ביטול</button>
